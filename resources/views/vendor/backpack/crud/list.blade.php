...
@section('content')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close close-modal-btn" aria-label="Close">&times;</button>
                ...
                <div class="modal-footer">
                    ...
                    <!-- The form action will be dynamically set using JS -->
    ...
    <div class="modal fade" id="deleteModalTrainer" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Confirm Deletion</h5>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary close-modal-btn" data-bs-dismiss="modal">Cancel</button>
                    <!-- The form action will be dynamically set using JS -->
                    <form id="delete-form-trainer" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger">Delete</button>
                    </form>
...
@section('after_scripts')
    ...
    <script>
        // Wait until the delete modal is about to show
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $('#deleteModal').on('show.bs.modal', function(event) {
                ...
                // Get the deleteId from the button's data-id attribute
                const deleteId = button.data('id');

                // Dynamically set the form action with the deleteId
                const form = document.getElementById('delete-form');
                form.setAttribute('action', '/admin/membership-detail/' + deleteId);
            });

            // Optional: Close the modal when the cancel button is clicked
        ...
        document.getElementById('deleteModalTrainer').addEventListener('show.bs.modal', function(event) {
            // Ensure the modal is displayed
            this.style.display = 'block';
            document.body.appendChild(this);
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Add event listener to handle modal opening
            $('#deleteModalTrainer').on('show.bs.modal', function(event) {
                // Get the button that triggered the modal
                const button = $(event.relatedTarget);
                // Get the deleteId from the button's data-id attribute
                const deleteId = button.data('id');
                ...
                // Dynamically set the form action with the deleteId
                const form = document.getElementById('delete-form-trainer');
                form.setAttribute('action', '/admin/trainer-detail/' + deleteId);
            });

            // Optional: Close the modal when the cancel button is clicked
            document.querySelectorAll('.close-modal-btn').forEach(function(button) {
                button.addEventListener('click', function() {
                    $('#deleteModalTrainer').modal('hide'); // Close the modal
                });
            });
        });
    </script>
