...
<h1 class="h1 text-white text-center text-uppercase my-4">{{ trans('backpack::base.login') }}</h1>
<form method="POST" action="{{ route('backpack.auth.login') }}" autocomplete="off" novalidate="">
    @csrf
    <div class="mb-3">
        <label class="form-label text-white" for="{{ $username }}">{{ trans('backpack::base.'.strtolower(config('backpack.base.authentication_column_name'))) }}</label>
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        <label class="form-check mb-0">
            ...
            <span class="form-check-label text-white">{{ trans('backpack::base.remember_me') }}</span>
        </label>
    ...
    <div class="form-footer">
        <button tabindex="5" type="submit" class="btn btn-white w-100 text-uppercase">{{ trans('backpack::base.login') }}</button>
    </div>
</form>
